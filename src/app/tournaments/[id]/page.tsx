...
export default function TournamentDetailsPage({
        ...
        if (data.ok && data.tournament) {
          ...
          if (data.tournament.type === "TEAM") {
            ...
            if (teamsData.ok && teamsData.teams) {
              const validTeams = teamsData.teams.filter(
                (t: Team) =>
          ...
          setMessage({
            text: data.message || "Error al cargar torneo",
            type: "error",
          });
        }
      } catch {
  ...
  const handleEnrollIndividual = async () => {
    ...
    try {
      ...
      if (data.ok) {
        setMessage({
          text: "¡Inscripción exitosa! Ya estás en el torneo.",
          type: "success",
        });
      } else {
        setMessage({
  ...
  const handleEnrollTeam = async () => {
    if (!selectedTeamId) {
      setMessage({
        text: "Debes seleccionar un equipo válido.",
        type: "error",
      });
      return;
    }
    ...
    try {
      ...
      if (data.ok) {
        ...
        setMessage({
          text: data.message || "Error al inscribir el equipo",
          type: "error",
        });
      }
    } catch {
    ...
    return (
      <main className="min-h-screen bg-[#09090b] text-white px-4 py-20 flex flex-col items-center">
        <div className="text-center py-20 border-2 border-dashed border-zinc-800 rounded-3xl bg-zinc-950/20 max-w-2xl w-full p-10">
          ...
          <Link
            href="/tournaments"
            className="px-6 py-3 bg-zinc-900 border border-zinc-800 rounded-xl text-sm font-bold uppercase hover:bg-zinc-800"
          >
            Volver
          </Link>
        </div>
  ...
  return (
    <main className="min-h-screen bg-[#09090b] text-white px-4 py-20 md:px-8 relative overflow-hidden">
      ...
      <div className="relative z-10 max-w-3xl mx-auto">
        <Link
          href="/tournaments"
          className="text-xs text-zinc-500 hover:text-arena-cyan transition-colors uppercase tracking-widest mb-6 inline-block font-bold"
        >
          ← Volver a Explorar
        </Link>
        ...
        <div className="rounded-3xl border border-zinc-800 bg-zinc-950/80 backdrop-blur overflow-hidden">
          ...
          <div className="h-32 bg-gradient-to-r from-zinc-900 via-arena-magenta/20 to-zinc-900 border-b border-zinc-800 flex items-end p-6">
            <span
              className={`px-3 py-1 rounded text-xs font-bold uppercase tracking-wider ${isRegistrationOpen ? "bg-green-500/20 text-green-400 border border-green-500/30" : "bg-zinc-800 text-zinc-400"}`}
            >
              {isRegistrationOpen
          ...
          <div className="p-8 md:p-10">
            ...
            <p className="text-arena-cyan text-lg font-bold mb-8">
              🎮 {tournament.game}
            </p>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-10">
              <div className="bg-zinc-900/50 p-4 rounded-xl border border-zinc-800/50">
                ...
                <p className="font-semibold">
                  {tournament.type === "INDIVIDUAL" ? "Individual" : "Equipos"}
                </p>
              </div>
              <div className="bg-zinc-900/50 p-4 rounded-xl border border-zinc-800/50">
                <p className="text-[10px] uppercase text-zinc-500 font-bold mb-1">
                ...
                <p className="font-semibold text-sm">
                  {tournament.elimination_mode === "SINGLE_ELIMINATION"
                    ? "Eliminación Simple"
                    : "Doble Eliminación"}
                </p>
              </div>
              <div className="bg-zinc-900/50 p-4 rounded-xl border border-zinc-800/50">
                ...
                <p className="font-semibold text-sm">
                  {tournament.region?.length
                    ? tournament.region.join(", ")
                    : "Global"}
                </p>
              </div>
              <div className="bg-zinc-900/50 p-4 rounded-xl border border-zinc-800/50">
                ...
                <p className="font-semibold">
                  ...
                  {tournament.type === "INDIVIDUAL" ? "jugadores" : "equipos"}
                </p>
              </div>
            </div>
            ...
            <div className="bg-zinc-900 border border-zinc-700 rounded-2xl p-6 md:p-8">
              ...
              {message && (
                <div
                  className={`p-4 rounded-xl mb-6 text-sm font-medium border ${message.type === "error" ? "bg-red-500/10 text-red-400 border-red-500/20" : "bg-green-500/10 text-green-400 border-green-500/20"}`}
                >
                  {message.text}
                </div>
              )}
              ...
              {!isRegistrationOpen ? (
                <button
                  disabled
                  className="w-full py-4 rounded-xl bg-zinc-800 text-zinc-500 font-bold uppercase cursor-not-allowed"
                >
                  ...
                  {myTeams.length === 0 ? (
                    <div className="text-center p-6 border border-zinc-800 rounded-xl bg-zinc-950/50">
                      <p className="text-zinc-400 text-sm mb-4">
                        No tienes equipos elegibles para este torneo. Debes ser
                        Capitán de un equipo, tener exactamente{" "}
                        {tournament.min_players_per_team} miembros unidos a tu
                        equipo, y no estar inscrito en otro torneo.
                      </p>
                      <Link
                        href="/teams"
                        className="text-arena-cyan hover:text-white font-bold text-sm uppercase transition-colors"
                      >
