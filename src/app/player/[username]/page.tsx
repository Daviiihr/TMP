  ...
  return (
    <main className="min-h-screen bg-[#09090b] text-zinc-100 flex items-center justify-center p-8">
      <div className="max-w-3xl w-full">
        ...
        <div className="bg-zinc-900 border rounded-2xl overflow-hidden shadow-2xl relative group transition-all duration-300" style={{ borderColor: `${themeColor}40` }}>
          {/* Banner */}
          ...
          <div className="px-8 pb-10 relative">
            <div className="flex justify-between items-end -mt-20 mb-6 relative z-10">
              ...
              <div className="px-5 py-2 rounded-full text-sm font-bold uppercase tracking-wider border shadow-lg backdrop-blur-sm" style={{ backgroundColor: `${themeColor}20`, color: themeColor, borderColor: `${themeColor}50` }}>
                {rank}
              </div>
            </div>
            ...
            <h1 className="text-4xl md:text-5xl font-bold text-white mb-2 flex items-center gap-4">
              {user.username}
              {user.role === 'ADMIN' && <span className="text-xs bg-red-500/20 text-red-400 px-3 py-1 rounded-md uppercase tracking-wider border border-red-500/30">Admin</span>}
              {user.role === 'CAPTAIN' && <span className="text-xs bg-yellow-500/20 text-yellow-400 px-3 py-1 rounded-md uppercase tracking-wider border border-yellow-500/30">Capitán</span>}
            </h1>
            ...
            <div className="bg-zinc-950/80 p-6 rounded-xl border border-zinc-800/50 backdrop-blur-md">
              <h3 className="text-zinc-500 uppercase tracking-widest text-xs font-bold mb-3 border-b border-zinc-800 pb-2" style={{ color: themeColor }}>Biografía</h3>
              <p className="text-zinc-200 text-base leading-relaxed whitespace-pre-wrap">
                {user.bio || <span className="text-zinc-600 italic">Este jugador aún no tiene una biografía. El misterio le rodea...</span>}
              </p>
            </div>
          </div>
        </div>
