...
export default function TournamentPanelPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const fetchTournamentDetails = async () => {
    try {
      ...
      if (data.tournament) {
        if (data.tournament.eliminationMode) setEliminationMode(data.tournament.eliminationMode);
      ...
      <div className="max-w-4xl mx-auto space-y-8">
        ...
        <section className="bg-zinc-900/40 backdrop-blur-xl border border-zinc-700/50 p-6 md:p-10 rounded-[2rem] shadow-[0_0_50px_-12px_rgba(0,242,254,0.15)] relative overflow-hidden transition-all hover:shadow-[0_0_50px_-12px_rgba(0,242,254,0.25)]">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6 mb-10 relative z-10">
            <div>
              <h2 className="text-3xl font-bold uppercase tracking-tight text-white mb-1">Participantes</h2>
              <p className="text-zinc-500 text-sm">Gestiona la lista antes de sortear los cruces.</p>
            </div>
            ...
            <div className="flex bg-zinc-950/80 p-1.5 rounded-xl border border-zinc-800 shadow-inner">
              <span className="px-4 py-2 rounded-lg bg-[#00f2fe]/10 text-xs font-black uppercase tracking-widest text-[#00f2fe] shadow-[0_0_20px_rgba(0,242,254,0.2)]">
                {eliminationMode === "SINGLE_ELIMINATION" ? "Eliminación Simple" : "Eliminación Doble"}
              </span>
            </div>
          </div>
          ...
          <div className="flex justify-center mb-10 relative z-10">
            ...
            >
              ...
              <svg className="w-6 h-6 transition-transform group-hover:rotate-180 duration-500" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12" /></svg>
          ...
          <div className="bg-zinc-950/60 border border-zinc-800/80 rounded-2xl p-6 mb-10 shadow-inner relative z-10">
            <div className="flex items-center justify-between mb-6 pb-4 border-b border-zinc-800/50">
              <h3 className="text-sm font-bold uppercase tracking-widest text-zinc-400">Listado Oficial</h3>
              <div className="flex items-center gap-2 bg-zinc-900 px-3 py-1.5 rounded-lg border border-zinc-800">
                <span className="w-2 h-2 rounded-full bg-[#00f2fe] animate-pulse"></span>
                <span className="text-zinc-300 text-sm font-bold">{participants.length} <span className="text-zinc-500 font-normal">Anotados</span></span>
              </div>
            </div>
            ...
            {participants.length === 0 ? (
              <div className="py-12 text-center flex flex-col items-center justify-center">
                <div className="w-16 h-16 rounded-full bg-zinc-900 flex items-center justify-center mb-4 border border-zinc-800/50 shadow-inner">
                  <svg className="w-8 h-8 text-zinc-600" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z" /></svg>
                </div>
                <p className="text-zinc-400 text-sm font-medium">El bracket está vacío.</p>
                <p className="text-zinc-600 text-xs mt-1">Haz clic en cargar inscritos para sincronizar.</p>
              </div>
            ) : (
              <ul className="space-y-3 max-h-[320px] overflow-y-auto pr-2 custom-scrollbar">
                {participants.map((p, idx) => (
                  <li key={p.id} className="group flex items-center justify-between bg-zinc-900/80 backdrop-blur-sm border border-zinc-800/80 p-4 rounded-xl hover:border-zinc-600 hover:bg-zinc-800/80 transition-all duration-300 hover:shadow-lg transform origin-left animate-fade-in-up" style={{ animationDelay: `${idx * 50}ms` }}>
                    <div className="flex items-center gap-4">
                      ...
                      <span className="font-bold text-zinc-200 text-lg group-hover:text-white transition-colors tracking-wide">{p.name}</span>
                    </div>
                    <button 
