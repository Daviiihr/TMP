...
export class CapacityObserver implements Observer<
  "enrollment:playerJoined" | "enrollment:teamJoined"
> {
  constructor(
  ...
  async update(
    eventName: "enrollment:playerJoined" | "enrollment:teamJoined",
    ...
    try {
      ...
      if (
        currentCount >= tournament.max_players &&
        tournament.status === "REGISTRATION"
      ) {
        console.log(
          `[CapacityObserver] Torneo ${tournamentId} lleno (${currentCount}/${tournament.max_players}). Cambiando estado a COMPLETED.`,
        );
        // Note: We use 'SYSTEM' as the user ID for automated actions
        await this.tournamentService.changeStatus(
          tournamentId,
          "COMPLETED",
      ...
      console.error(
        `[CapacityObserver] Error verificando capacidad del torneo ${tournamentId}:`,
        error,
      );
    }
  }
