...
export class LoggerObserver implements Observer<EventKey> {
  async update<T extends EventKey>(
    eventName: T,
    data: AppEvents[T],
  ): Promise<void> {
    ...
    switch (eventName) {
      case "tournament:statusChanged": {
        ...
        console.log(
          `[${timestamp}] 🏆 ${eventName} — Torneo ${payload.tournamentId} cambió de ${payload.oldStatus} → ${payload.newStatus} (Usuario: ${payload.userId})`,
        );
        break;
      }
      case "enrollment:playerJoined": {
        ...
        console.log(
          `[${timestamp}] 👤 ${eventName} — Jugador ${payload.userId} se inscribió al torneo ${payload.tournamentId}`,
        );
        break;
      }
      case "enrollment:teamJoined": {
        ...
        console.log(
          `[${timestamp}] 👥 ${eventName} — Equipo ${payload.teamId} se inscribió al torneo ${payload.tournamentId}`,
        );
        break;
      }
      case "team:created": {
        ...
        console.log(
          `[${timestamp}] 🛡️ ${eventName} — Equipo "${payload.name}" (${payload.teamId}) creado por el capitán ${payload.captainId}`,
        );
        break;
      }
      default:
