...
export class TeamRepository {
  ...
  async searchTeams(query: string): Promise<(Team & { member_count: number })[]> {
...
export class TeamRepository {
  ...
  async create(data: { name: string; captain_id: string; size: number }): Promise<Team> {
    const result = await this.pool.query<Team>(
      `INSERT INTO teams (name, captain_id, size) 
       VALUES ($1, $2, $3) 
       RETURNING id, name, captain_id, size, created_at`,
      [data.name, data.captain_id, data.size]
    );
    return result.rows[0];
  }
  ...
  async findById(id: string): Promise<Team | null> {
    const result = await this.pool.query<Team>(
      `SELECT id, name, captain_id, size, created_at FROM teams WHERE id = $1`,
      [id]
    );
    return result.rows[0] || null;
  }
  ...
  async findByCaptain(captainId: string): Promise<Team[]> {
    const result = await this.pool.query<Team>(
      `SELECT id, name, captain_id, size, created_at FROM teams WHERE captain_id = $1`,
      [captainId]
    );
    return result.rows;
  }
  ...
  async searchTeams(query: string): Promise<(Team & { member_count: number })[]> {
    const result = await this.pool.query(
      `SELECT t.id, t.name, t.captain_id, t.size, t.created_at, 
              COUNT(tm.user_id)::int as member_count
       FROM teams t
       LEFT JOIN team_members tm ON t.id = tm.team_id
       WHERE t.name ILIKE $1
