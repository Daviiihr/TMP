...
export class BracketRepository {
  ...
  async saveBracket(
    ...
    eliminationMode:
      "SINGLE_ELIMINATION" | "DOUBLE_ELIMINATION" = "SINGLE_ELIMINATION",
  ) {
        ...
        if (m.isBye && nextPgId) {
          ...
          if (advancingPlayer) {
            const participantField =
              m.nextMatchSlot === 1 ? "participant1_id" : "participant2_id";
            const nameField =
              m.nextMatchSlot === 1 ? "participant1_name" : "participant2_name";
            await client.query(
  ...
  async getLiveBracket(
    tournamentId: string,
  ): Promise<{ bracketData: BracketResult; eliminationMode: string } | null> {
    ...
    const updateMatch = (m: Match) => {
      ...
      if (liveData) {
        if (liveData.participant1_id)
          m.player1 = {
            id: liveData.participant1_id,
            name: liveData.participant1_name,
          };
        else m.player1 = null;

        if (liveData.participant2_id)
          m.player2 = {
            id: liveData.participant2_id,
            name: liveData.participant2_name,
          };
        else m.player2 = null;

        // Extending the Match interface dynamically to include status and winner in the JSON payload
        (m as any).status = liveData.status;
        (m as any).winnerId = liveData.winner_id;
  ...
  async updateMatchWinner(
    ...
    try {
      ...
      if (loserId && matchInfo.loser_next_match_id) {
        const loserField =
          matchInfo.loser_next_match_slot === 1
            ? "participant1_id"
            : "participant2_id";
        const loserNameField =
          matchInfo.loser_next_match_slot === 1
            ? "participant1_name"
            : "participant2_name";
