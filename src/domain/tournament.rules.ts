...
export const ELIMINATION_MODES = [
  "SINGLE_ELIMINATION",
  "DOUBLE_ELIMINATION",
] as const;
...
export function validateTournamentCreation(
  ...
  if (!isTournamentType(type)) {
    throw new Error(
      `Tipo de torneo invalido. Debe ser: ${TOURNAMENT_TYPES.join(", ")}.`,
    );
  }

  if (!isEliminationMode(eliminationMode)) {
    throw new Error(
      `Modalidad invalida. Debe ser: ${ELIMINATION_MODES.join(", ")}.`,
    );
  }
  ...
  if (
    ...
    maxPlayers < MIN_TOURNAMENT_PARTICIPANTS
  ) {
    throw new Error(
      `El torneo debe permitir al menos ${MIN_TOURNAMENT_PARTICIPANTS} participantes.`,
    );
  }
