  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setMessage({
          type: "error",
          text: data.message || "Error al crear el equipo",
        });
      }
    } catch {
  ...
  return (
    <div className="bg-zinc-900/50 border border-zinc-800 p-8 rounded-3xl">
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        {message && (
          <div
            className={`p-4 rounded-lg text-sm font-medium ${message.type === "success" ? "bg-green-500/10 text-green-500 border border-green-500/20" : "bg-red-500/10 text-red-500 border border-red-500/20"}`}
          >
            {message.text}
          </div>
        )}
