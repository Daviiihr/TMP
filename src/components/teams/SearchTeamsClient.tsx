...
export default function SearchTeamsClient({
  initialTeams,
}: {
  ...
  const loadTeams = async (searchTerm = "", preserveMessage = false) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setMessage({
          type: "error",
          text: data.message ?? "Error al buscar equipos",
        });
      }
    } catch {
  ...
  const handleJoin = async (teamId: string) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setMessage({
          type: "error",
          text: data.message ?? "Error al intentar unirse",
        });
      }
    } catch {
  ...
  return (
    <main className="min-h-screen bg-[#09090b] text-zinc-100 p-4 md:p-8">
      <div className="max-w-4xl mx-auto">
        <header className="flex items-center justify-between mb-12">
          <div>
            ...
            <p className="text-zinc-400">
              Explora todos los equipos creados y filtra por nombre
            </p>
          </div>
          <Link
            href="/dashboard"
        ...
        {message && (
          <div
            className={`mb-8 p-4 rounded-lg text-sm font-medium ${message.type === "success" ? "bg-green-500/10 text-green-500 border border-green-500/20" : "bg-red-500/10 text-red-500 border border-red-500/20"}`}
          >
            {message.text}
          </div>
        )}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {teams.length > 0
            ? teams.map((team) => (
                <div
                  key={team.id}
                  className="bg-zinc-900/50 border border-zinc-800 p-6 rounded-2xl flex items-center justify-between group hover:border-arena-cyan transition-all"
                >
                  <div>
