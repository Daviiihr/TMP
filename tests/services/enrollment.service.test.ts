...
describe('EnrollmentService', () => {
  ...
  describe('enrollPlayerInTournament', () => {
    it('should throw if tournament not found', async () => {
      ...
      await expect(enrollmentService.enrollPlayerInTournament('u1', 't1'))
        .rejects.toThrow('Torneo no encontrado');
    });

    it('should throw if tournament is not DRAFT or REGISTRATION', async () => {
      mockTournamentRepo.getById.mockResolvedValueOnce({ status: 'IN_PROGRESS' } as any);
      await expect(enrollmentService.enrollPlayerInTournament('u1', 't1'))
        .rejects.toThrow('cerrado');
    });

    it('should throw if tournament is not INDIVIDUAL', async () => {
      mockTournamentRepo.getById.mockResolvedValueOnce({ status: 'REGISTRATION', type: 'TEAM' } as any);
      await expect(enrollmentService.enrollPlayerInTournament('u1', 't1'))
        .rejects.toThrow('solo para equipos');
    });

    it('should throw if player is already enrolled', async () => {
      mockTournamentRepo.getById.mockResolvedValueOnce({ status: 'REGISTRATION', type: 'INDIVIDUAL' } as any);
      mockPool.query.mockResolvedValueOnce({ rows: [{ '?column?': 1 }] } as any); // isEnrolled = true
    ...
    it('should throw if tournament is full', async () => {
      mockTournamentRepo.getById.mockResolvedValueOnce({ status: 'REGISTRATION', type: 'INDIVIDUAL', max_players: 8 } as any);
    ...
    it('should enroll player successfully', async () => {
      ...
      expect(mockEmitter.emit).toHaveBeenCalledWith('enrollment:playerJoined', { userId: 'u1', tournamentId: 't1' });
    });
  });
  ...
  describe('enrollTeamInTournament', () => {
    it('should throw if team not found', async () => {
      ...
      await expect(enrollmentService.enrollTeamInTournament('team1', 'tourn1'))
    ...
    it('should throw if team already in a tournament', async () => {
      mockTeamRepo.findById.mockResolvedValueOnce({ id: 'team1', tournament_id: 'other-tourn' } as any);
      mockTournamentRepo.getById.mockResolvedValueOnce({ status: 'REGISTRATION', type: 'TEAM' } as any);
    ...
    it('should throw if team sizes mismatch', async () => {
      mockTeamRepo.findById.mockResolvedValueOnce({ id: 'team1', size: 5 } as any);
      mockTournamentRepo.getById.mockResolvedValueOnce({ status: 'REGISTRATION', type: 'TEAM', min_players_per_team: 4, max_players: 8 } as any);
    ...
    it('should throw if team is incomplete', async () => {
      mockTeamRepo.findById.mockResolvedValueOnce({ id: 'team1', size: 5 } as any);
      mockTournamentRepo.getById.mockResolvedValueOnce({ status: 'REGISTRATION', type: 'TEAM', min_players_per_team: 5, max_players: 8 } as any);
      mockTournamentRepo.getEnrollmentCount.mockResolvedValueOnce(0);
    ...
    it('should enroll team successfully', async () => {
      ...
      const res = await enrollmentService.enrollTeamInTournament('team1', 'tourn1');
      
      expect(res.success).toBe(true);
      expect(mockTeamRepo.assignToTournament).toHaveBeenCalledWith('team1', 'tourn1');
      expect(mockEmitter.emit).toHaveBeenCalledWith('enrollment:teamJoined', { teamId: 'team1', tournamentId: 'tourn1' });
    });
  });
});
