  ...
  describe("recalculateUserStats", () => {
    it("should calculate points and call upsert", async () => {
      mockPool.query.mockImplementation((queryStr: string) => {
        if (queryStr.includes("SELECT country"))
          return { rows: [{ country: "CL" }] };
        if (queryStr.includes("SELECT mr.id")) return { rows: [{ id: "m1" }] };
        if (
          queryStr.includes("indQuery") ||
          queryStr.includes("m.winner_id = $1")
        )
          return { rows: [{ wins: "5", losses: "2" }] };
        if (
          queryStr.includes("teamQuery") ||
          queryStr.includes("m.winner_id = tm.team_id")
        )
          return { rows: [{ wins: "2", losses: "1" }] };
        return { rows: [] };
      });
    ...
    it("should handle zero results safely", async () => {
      ...
      expect(
        mockRankingRepo.recalculateCountryPositions,
  ...
  describe("recalculateAllRankings", () => {
    it("should iterate over all users", async () => {
      mockPool.query.mockImplementation((queryStr: string) => {
        ...
        if (queryStr.includes("m.winner_id"))
          return { rows: [{ wins: "1", losses: "0" }] };
        return { rows: [] };
      });
      ...
      expect(mockRankingRepo.recalculateCountryPositions).toHaveBeenCalledTimes(
        1,
      );
    });
  });
