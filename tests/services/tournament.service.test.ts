...
describe("TournamentService", () => {
  ...
  describe("createTournament", () => {
    ...
    it("should throw if name already exists", async () => {
      ...
      await expect(
        tournamentService.createTournament(validInput),
    ...
    it("should throw if validation fails", async () => {
      ...
      await expect(
        tournamentService.createTournament(invalidInput),
  ...
  describe("changeStatus", () => {
    ...
    it("should throw on invalid status", async () => {
      await expect(
        tournamentService.changeStatus("tourn-1", "INVALID", "org-1"),
    ...
    it("should throw if tournament not found", async () => {
      ...
      await expect(
        tournamentService.changeStatus("tourn-1", "IN_PROGRESS", "org-1"),
    ...
    it("should throw if user is not organizer or system", async () => {
      ...
      await expect(
        tournamentService.changeStatus("tourn-1", "IN_PROGRESS", "hacker"),
    ...
    it("should return immediately if status is the same", async () => {
      ...
      const result = await tournamentService.changeStatus(
        "tourn-1",
        "IN_PROGRESS",
        "org-1",
      );
    ...
    it("should prevent setting REGISTRATION if another is active", async () => {
      ...
      mockPool.query.mockResolvedValue({
        rows: [{ name: "Another Active" }],
      } as any);

      await expect(
        tournamentService.changeStatus("tourn-1", "REGISTRATION", "org-1"),
