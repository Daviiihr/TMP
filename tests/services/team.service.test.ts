...
describe("TeamService", () => {
  ...
  describe("createTeam", () => {
    it("should throw if size < 1", async () => {
      await expect(teamService.createTeam("Fnatic", "c1", 0)).rejects.toThrow(
        "El tamaño del equipo debe ser al menos 1",
      );
    });
    ...
    it("should create team, add captain, update role, and emit event", async () => {
      ...
      expect(mockEmitter.emit).toHaveBeenCalledWith("team:created", {
        teamId: "t1",
        captainId: "c1",
        name: "Fnatic",
      });
  ...
  describe("joinTeam", () => {
    it("should throw if team not found", async () => {
      mockTeamRepo.findById.mockResolvedValueOnce(null);
      await expect(teamService.joinTeam("t1", "u1")).rejects.toThrow(
