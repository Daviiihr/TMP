...
describe('AuthService', () => {
  ...
  describe('verifyPassword', () => {
    ...
    it('should return error if account is temporarily locked', async () => {
      const lockedUser = { ...mockUser, locked_until: new Date(Date.now() + 10000) };
    ...
    it('should handle failed login and increment attempts', async () => {
      ...
      const result = await authService.verifyPassword(mockUser, 'wrongpassword');
      
      expect(result.ok).toBe(false);
      expect(result.status).toBe(401);
      expect(mockUserRepo.updateLoginAttempts).toHaveBeenCalledWith('user-1', 1, null);
    });
    ...
    it('should return success and reset attempts on correct password', async () => {
      ...
      const result = await authService.verifyPassword(mockUser, 'correctpassword');
