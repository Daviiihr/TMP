...
describe("RankingPage", () => {
  ...
  it("renders loading state initially and then lists rankings", async () => {
    ...
    (global.fetch as any).mockImplementation((url: string) => {
      if (url.includes("/api/auth/login")) {
        return Promise.resolve({
          ok: true,
          json: async () => ({ user: { username: "Faker" } }),
        });
      }
      return fetchPromise;
    });
    ...
    expect(
      screen.getByText("Cargando la arena de competición..."),
    ).toBeInTheDocument();
  ...
  it("filters rankings by search query", async () => {
    ...
    // Search for 'Caps'
    fireEvent.change(
      screen.getByPlaceholderText("Escribe el nombre de usuario..."),
      { target: { value: "Caps" } },
    );
  ...
  it("recalculates rankings when clicking the button", async () => {
    (global.fetch as any).mockImplementation((url: string) => {
      ...
      if (url.includes("/api/ranking/recalculate")) {
        return Promise.resolve({
          ok: true,
          json: async () => ({ message: "Recalculated successfully" }),
        });
      }
      return Promise.resolve({
        ok: true,
    ...
    await waitFor(() => {
      ...
      expect(global.fetch).toHaveBeenCalledWith(
        "/api/ranking/recalculate?seed=true",
        expect.any(Object),
      );
    });
  });
