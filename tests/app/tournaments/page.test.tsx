...
describe("TournamentsPage", () => {
  ...
  it("renders loading state initially and then lists tournaments", async () => {
    ...
    expect(
      screen.getByText("Cargando torneos de la comunidad..."),
    ).toBeInTheDocument();
  ...
  it("filters tournaments based on search, type and status", async () => {
    ...
    // Filter by name
    fireEvent.change(
      screen.getByPlaceholderText("Ej: Copa Smash, League of Legends..."),
      { target: { value: "Smash" } },
    );

    expect(screen.getByText("Copa Smash")).toBeInTheDocument();
    expect(
      screen.queryByText("League of Legends Worlds"),
  ...
  it("displays empty state if no matches found", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText("No se encontraron torneos."),
      ).toBeInTheDocument();
    });
  });
});
