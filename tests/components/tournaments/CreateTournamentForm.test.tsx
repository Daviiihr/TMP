...
describe('CreateTournamentForm', () => {
  ...
  it('renders step 1 and validates required fields', () => {
    ...
    expect(screen.getByText('Por favor completa todos los campos de información básica.')).toBeInTheDocument();
  });
  ...
  it('progresses through steps and submits successfully', async () => {
    ...
    // Step 1
    fireEvent.change(screen.getByPlaceholderText('Ej. Global Masters 2026'), { target: { value: 'My Tourney' } });
    fireEvent.change(screen.getAllByRole('combobox')[0], { target: { value: 'Valorant' } });
    fireEvent.change(screen.getByPlaceholderText('Ej. NA, EU, LATAM'), { target: { value: 'LATAM' } });
    fireEvent.change(screen.getAllByRole('combobox')[1], { target: { value: 'SINGLE_ELIMINATION' } });
    
    fireEvent.click(screen.getByText('Siguiente →'));
    
    // Step 2
    expect(screen.getByText('Restricciones de Inscripción')).toBeInTheDocument();
