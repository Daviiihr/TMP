...
async function runTests() {
  ...
  try {
    ...
    try {
      ...
      console.log(
        "❌ Player enrolled in individual tournament: FAILED - " +
          getErrorMessage(e),
      );
    }
    ...
    const team5 = await teamRepo.create({
      name: "Dream Team",
      captain_id: playerId,
      size: 5,
    });
    try {
      ...
      console.log(
        "❌ Team (size 5) enrolled in tournament (size 5): FAILED - " +
          getErrorMessage(e),
      );
    }
    ...
    try {
      ...
      console.log(
        "❌ Team (size 3) enrolled in tournament (size 5): SHOULD HAVE FAILED",
      );
    } catch (e: unknown) {
      console.log(
        "✅ Team (size 3) enrolled in tournament (size 5): FAILED AS EXPECTED - " +
          getErrorMessage(e),
      );
    }
    ...
    try {
      ...
      console.log(
        "✅ Player enrolled in TEAM tournament: FAILED AS EXPECTED - " +
          getErrorMessage(e),
      );
    }
    ...
    try {
      ...
      console.log(
        "❌ Team enrolled in INDIVIDUAL tournament: SHOULD HAVE FAILED",
      );
    } catch (e: unknown) {
      console.log(
        "✅ Team enrolled in INDIVIDUAL tournament: FAILED AS EXPECTED - " +
          getErrorMessage(e),
      );
    }
