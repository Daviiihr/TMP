...
describe("BracketRepository", () => {
  ...
  describe("saveBracket", () => {
    it("should save bracket with transaction and return id", async () => {
      mockClient.query.mockImplementation((queryStr: string) => {
        if (queryStr.includes("gen_random_uuid"))
          return { rows: [{ uuid: "generated-uuid" }] };
        if (queryStr.includes("INSERT INTO brackets"))
          return { rows: [{ id: "bracket-id" }] };
        return { rows: [] };
      });
    ...
    it("should rollback on error", async () => {
      ...
      await expect(
        bracketRepo.saveBracket("t1", "user1", { rounds: [] } as any),
  ...
  describe("updateMatchWinner", () => {
    it("should update match winner and move players", async () => {
      ...
      mockClient.query.mockImplementation((queryStr: string) => {
        if (queryStr.includes("SELECT participant1_id"))
          return { rows: [matchInfo] };
        return { rows: [] };
      });
    ...
    it("should rollback if match not found", async () => {
      ...
      await expect(
        bracketRepo.updateMatchWinner("t1", "m1", "p1"),
